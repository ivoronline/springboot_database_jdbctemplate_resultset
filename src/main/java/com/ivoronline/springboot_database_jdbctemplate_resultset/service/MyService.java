...
public class MyService {
  ...
  public PersonDTO selectRecord(Integer id) throws SQLException {
    ...
    ResultSet  resultSet  = statement.executeQuery("SELECT * FROM PERSON WHERE ID = " + id); //Parameters
  ...
  public List<PersonDTO> selectRecords() throws SQLException {
    ...
    Connection connection = DataSourceUtils.getConnection(dataSource);
    Statement  statement  = connection.createStatement();
    ResultSet  resultSet  = statement.executeQuery("SELECT * FROM PERSON PERSON WHERE ID >= 1"); //No Parameters
